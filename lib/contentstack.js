...
/**
 * Contentstack SDK often rejects with a plain object { error_message, errors, ... }.
 * @param {unknown} err
 * @returns {Record<string, unknown>}
 */
function contentstackErrShape(err) {
  if (err == null) return { kind: "nullish" };
  if (typeof err !== "object") return { kind: "primitive", value: String(err) };
  const o = {};
  for (const k of [
    "error_message",
    "message",
    ...
    if (k in err && (/** @type {Record<string, unknown>} */ (err))[k] != null) {
      o[k] = (/** @type {Record<string, unknown>} */ (err))[k];
    }
  }
  if (Object.keys(o).length === 0) o.stringFallback = String(err);
  return o;
}
 ...
 * Override with CONTENTSTACK_HOME_CONTENT_TYPE_UID when your stack uses a different UID
 * (e.g. some stacks use `home_page` instead of `homepage`).
 * @returns {string}
 */
...
export async function getEntries(contentTypeUid) {
  ...
  agentDebugLog({
    ...
    data: {
      ...
      homeTypeUidOverride:
        process.env.CONTENTSTACK_HOME_CONTENT_TYPE_UID?.trim() || null,
    },
  });
  // #endregion
  ...
  try {
    ...
    const msg =
      err && typeof err === "object" && "message" in err
        ...
        : String(err);
    const csShape = contentstackErrShape(err);
    // #region agent log
    agentDebugLog({
      hypothesisId: "H1",
      ...
      data: {
        contentTypeUid,
        errorMessage: msg,
        contentstackError: csShape,
      },
    });
    agentDebugLog({
      hypothesisId: "H3",
      location: "lib/contentstack.js:getEntries:err-detail",
      message: "getEntries API rejection detail",
      data: {
        contentTypeUid,
        environment: process.env.CONTENTSTACK_ENVIRONMENT ?? null,
        contentstackError: csShape,
